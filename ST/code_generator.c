...
{
    if (!expr_node)
        return "%s"; // Default para string ou erro
    ...
    else if (expr_node->type == ID_NODE)
    {
        ...
        {
            if (strcmp(id_node->entry->tipo, "int") == 0)
                return "%d";
            if (strcmp(id_node->entry->tipo, "float") == 0)
                return "%lf";
            if (strcmp(id_node->entry->tipo, "double") == 0)
                return "%lf";
            if (strcmp(id_node->entry->tipo, "char") == 0)
                return "%c";
            if (strcmp(id_node->entry->tipo, "string") == 0)
                return "%s";
            if (strcmp(id_node->entry->tipo, "boolean") == 0)
                return "%d";
        }
    }
...
static void gerarExpressao(NoAST *no, FILE *saida)
{
    ...
    case ASSIGN_NODE:
    {
        ...
        if (!assign_node || !assign_node->target_entry)
            return;
        ...
        {
            if (strcmp(target_symbol->tipo, "int") == 0)
                c_type = "int";
            else if (strcmp(target_symbol->tipo, "float") == 0)
                c_type = "float";
            else if (strcmp(target_symbol->tipo, "double") == 0)
                c_type = "double";
            else if (strcmp(target_symbol->tipo, "char") == 0)
                c_type = "char";
            else if (strcmp(target_symbol->tipo, "string") == 0)
                c_type = "char*";
            else if (strcmp(target_symbol->tipo, "boolean") == 0)
                c_type = "int";
        }
        else
        {
    ...
    case ARITHM_NODE:
    {
        ...
        case OP_DIVISAO:
            ...
            break;
        default:
            fprintf(saida, " /* UNKNOWN_ARITHM_OP */ ");
            break;
        }
    ...
    case LOGIC_OP_NODE:
    {
        ...
        else
        {
            ...
            case OP_OR:
                ...
                break;
            default:
                fprintf(saida, " /* UNKNOWN_LOGIC_OP */ ");
                break;
            }
    ...
    case FUNC_CALL_NODE:
    {
        ...
        if (strcmp(call_node->entry->nome, "puts") == 0 || strcmp(call_node->entry->nome, "print") == 0)
        {
            ...
            while (temp_iter_format)
            {
                ...
                { // somente adiciona formato se a expressao existe
            ...
            while (temp_iter_value)
            {
                ...
                { // somente adiciona valor se a expressao existe
        ...
        else if (strcmp(call_node->entry->nome, "scan") == 0 || strcmp(call_node->entry->nome, "gets") == 0)
        {
            ...
            if (current_arg && current_arg->expr && current_arg->expr->type == ID_NODE)
            {
                ...
                if (!isVarDeclaredInCurrentCScope(target_symbol->nome))
                {
                    ...
                    if (target_symbol->tipo != NULL)
                    {
                        if (strcmp(target_symbol->tipo, "float") == 0)
                            scan_type = "float";
                        else if (strcmp(target_symbol->tipo, "double") == 0)
                            scan_type = "double";
                        else if (strcmp(target_symbol->tipo, "char") == 0)
...
static void gerarStatement(NoAST *no, FILE *saida)
{
    ...
    case BASIC_NODE:
        if (no->esquerda)
            gerarStatement(no->esquerda, saida);
        if (no->direita)
            gerarStatement(no->direita, saida);
        break;
    case ASSIGN_NODE:
    case FUNC_CALL_NODE:
    case ID_NODE:
    ...
    case IF_NODE:
    {
        ...
        // Não limpar variáveis declaradas - if não cria novo escopo
    ...
    case ELSIF_NODE:
    {
        ...
        fprintf(saida, ") {\n");
        current_indent_level++;
        // Não limpar variáveis declaradas - else if não cria novo escopo
    ...
    case ELSE_NODE:
    {
        ...
        fprintf(saida, "else {\n");
        current_indent_level++;
        // Não limpar variáveis declaradas - else não cria novo escopo
    ...
    case FOR_IN_NODE:
    {
        ...
        if (for_node->collection_expr->type == RANGE_NODE)
        {
            ...
            if (!range_node || !iter_id_node || !iter_id_node->name)
                return;
    ...
    case WHILE_NODE:
    {
        ...
        current_indent_level++;
        // Não limpar variáveis declaradas - while não cria novo escopo
